  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (method === 'phone') {
        if (!isCodeSent) {
            ...
            if (!formData.phone || formData.phone.length < 10) {
                notificationService.showNotification("Invalid Phone", { body: "Please enter a valid number." });
            ...
            const code = otp.join('');
            if (code.length !== 6) {
                notificationService.showNotification("Invalid Code", { body: "Please enter the 6-digit code." });
                hapticFeedback.error();
                return;
            }
    ...
    if (method === 'email') {
        ...
        if (!formData.email || !formData.password) {
             notificationService.showNotification("Missing Credentials", { body: "Please enter both email and password." });
             hapticFeedback.error();
             return;
        }
        ...
        if (mode === 'signup') {
            if (formData.password !== formData.confirmPassword) {
                notificationService.showNotification("Password Error", { body: "Passwords do not match." });
                hapticFeedback.error();
                return;
            }
            if (!formData.name.trim()) {
                notificationService.showNotification("Missing Info", { body: "Please enter your full name." });
                hapticFeedback.error();
                return;
            }
        }
    ...
    try {
        // Attempt Firebase Auth with timeout to prevent hanging
        const authPromise = mode === 'signup' 
            ? createUserWithEmailAndPassword(auth, formData.email, formData.password)
            : signInWithEmailAndPassword(auth, formData.email, formData.password);
            
        // Race against a timeout
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject({ code: 'auth/timeout' }), 5000));
        
        const userCredential: any = await Promise.race([authPromise, timeoutPromise]);

        if (mode === 'signup' && userCredential?.user) {
        ...
        // Identify strictly user input errors
        const isUserError = error.code === 'auth/email-already-in-use' ||
        ...
        if (!isUserError) {
            // If it's NOT a specific user error (e.g. invalid API key, network timeout, config error),
            // we fallback to Demo Mode immediately.
            await activateDemoMode();
             ...
             notificationService.showNotification("Error", { body: msg });
             hapticFeedback.error();
        }
    } finally {
        setIsLoading(false);
