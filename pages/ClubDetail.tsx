...
// Mock data
const CLUBS_DATA: Record<string, Partial<Club>> = {
  '1': {
    id: '1',
    ownerId: 'system',
    ...
    description: 'We run the city when it sleeps. Join us for late-night 5Ks and post-run smoothies. All paces welcome, headlamps recommended!',
    members: 1240,
    location: 'Central Park, NY',
    rules: ['No headphones during group runs', 'Wait for the last runner', 'Bring positive vibes'],
    isMember: true,
  ...
  '2': {
    ...
    description: 'Dedicated to finding your perfect doubles partner. Weekly tournaments and casual rally sessions.',
    members: 85,
    location: 'City Courts',
    rules: ['White attire preferred', 'Bring your own balls'],
    isMember: false,
...
const MOCK_CLUB_EVENTS: SportEvent[] = [
    {
        id: 'e1',
        hostId: 'system',
        hostName: 'Club Admin',
        hostAvatar: '',
        title: 'Weekly Night Run',
        sport: SportType.RUNNING,
        date: 'Tonight',
        time: '22:00',
        location: 'Columbus Circle',
        description: 'Our signature 5K loop around the park. Pace groups available.',
        attendees: 42,
        attendeeAvatars: ['https://i.pravatar.cc/150?u=1', 'https://i.pravatar.cc/150?u=2'],
        isJoined: true,
    ...
    },
    {
        ...
        date: 'Saturday',
        time: '07:00',
        location: 'Central Park Engineers Gate',
        description: '20 mile training run. Hydration stations provided.',
        attendees: 18,
        attendeeAvatars: ['https://i.pravatar.cc/150?u=3', 'https://i.pravatar.cc/150?u=4'],
        isJoined: false,
...
const MOCK_CHAT_MESSAGES = [
  { id: '1', sender: 'Alex', text: 'Anyone up for a run tonight?', time: '10:30 AM', avatar: 'https://i.pravatar.cc/150?u=1', read: true },
  { id: '2', sender: 'Sarah', text: 'I will be there! bringing a friend.', time: '10:32 AM', avatar: 'https://i.pravatar.cc/150?u=2', read: true },
  { id: '3', sender: 'Mike', text: 'Don\'t forget your reflective gear guys!', time: '10:45 AM', avatar: 'https://i.pravatar.cc/150?u=3', read: true },
];

const MOCK_MEMBERS = [
    { id: '1', name: 'Alex Smith', avatar: 'https://i.pravatar.cc/150?u=1', role: 'Member' },
    { id: '2', name: 'Sarah Connor', avatar: 'https://i.pravatar.cc/150?u=2', role: 'Member' },
    { id: '3', name: 'Mike Ross', avatar: 'https://i.pravatar.cc/150?u=3', role: 'Admin' }
];

const MOCK_APPLICANTS = [
    { id: 'a1', name: 'John Doe', avatar: 'https://i.pravatar.cc/150?u=a1', bio: 'Love running!' },
    { id: 'a2', name: 'Jane Smith', avatar: 'https://i.pravatar.cc/150?u=a2', bio: 'Looking for a group.' },
    { id: 'a3', name: 'Bob Wilson', avatar: 'https://i.pravatar.cc/150?u=a3', bio: 'Marathon training.' }
];
...
export const ClubDetail: React.FC = () => {
  ...
  const initialClub = (location.state as { club: Club })?.club || (clubId ? CLUBS_DATA[clubId] : null);
  const [club, setClub] = useState<any>(initialClub);
  
  const [activeTab, setActiveTab] = useState<'info' | 'events' | 'chat' | 'dashboard'>('info');
  const [events, setEvents] = useState(MOCK_CLUB_EVENTS);
      ...
      <div className={`px-6 py-4 border-b backdrop-blur-md sticky top-0 z-20 ${isLight ? 'border-slate-200/50 bg-white/80' : 'border-white/5 bg-black/80'}`}>
         <div className={`flex rounded-xl p-1 backdrop-blur-md ${isLight ? 'bg-slate-100' : 'bg-white/5'}`}>
            {['info', 'events', 'chat'].map((tab) => (
                ...
                >
                    {tab}
                    {tab === 'chat' && membershipStatus !== 'member' && <span className="ml-1">🔒</span>}
                </button>
            ))}
            {isOwner && (
                <button 
        ...
        {activeTab === 'chat' && (
          <div className="h-[calc(100vh-350px)] flex flex-col">
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 mb-4 no-scrollbar">
               <div className={`text-center text-xs my-4 ${isLight ? 'text-slate-400' : 'text-white/30'}`}>Today</div>
               {messages.map((msg) => {
                 const isMe = msg.sender === 'You';
                 return (
                   <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''} animate-slide-up`}>
                     <img src={msg.avatar} className="w-8 h-8 rounded-full self-end mb-1 object-cover ring-1 ring-white/20" alt={msg.sender} />
                     <div className={`max-w-[80%]`}>
                       {!isMe && <div className={`text-[10px] ml-1 mb-1 ${isLight ? 'text-slate-400' : 'text-white/40'}`}>{msg.sender}</div>}
                       <div className={`
                          px-4 py-2.5 rounded-2xl text-sm shadow-sm backdrop-blur-md
                          ${isMe 
                            ? 'bg-neon-blue text-black rounded-br-none' 
                            : (isLight ? 'bg-white/60 text-slate-800 border border-slate-200/50 rounded-bl-none' : 'bg-white/10 text-white rounded-bl-none border border-white/5')
                          }
                       `}>
                         {msg.text}
                       </div>
            ...
            <div className="relative flex items-end gap-2">
               <button className={`p-3 rounded-xl transition ${isLight ? 'bg-white/50 border border-slate-200 text-slate-500 hover:bg-slate-50' : 'bg-white/5 text-white/60 hover:text-white hover:bg-white/10'}`}>
                 <ImageIcon size={20} />
               </button>
               ...
               <button 
                 ...
                 className="p-3 rounded-xl bg-neon-blue text-black hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-lg shadow-neon-blue/20"
               >
                 <Send size={20} />
               </button>
            </div>
